...
export async function getBejelentettPosztok(_req: Request, res: Response) {
  ...
  try {
    ...
    console.error('Error in getBejelentettPosztok:', e);
    return res.status(500).json({ uzenet: "Szerver hiba", error: e?.message });
  } finally {
    await connection.end();
  }
}

export async function getOsszesEgyeniReport(_req: Request, res: Response) {
  const connection = await mysql.createConnection(config.database);
  try {
    const [resultSets] = await connection.query<any[]>("CALL get_osszes_egyeni_report()");
    const rows = resultSets[0];
    return res.json(rows);
  } catch (e: any) {
    console.error('Error in getOsszesEgyeniReport:', e);
    return res.status(500).json({ uzenet: "Szerver hiba", error: e?.message });
  } finally {
    await connection.end();
  }
