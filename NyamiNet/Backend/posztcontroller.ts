...
export async function getReszletesPoszt(req: Request, res: Response) {
    ...
    try {
        ...
        const hozzavalok = eredmenysorok[1][0].hozzavalok;
...
export async function ujPoszt(req: Request, res: Response) {
    ...
    try {
        ...
        //hozzavalok
        if (Array.isArray(hozzavalok)) {
            for (const hozzavaloId of hozzavalok) {
                await connection.query(
                    "INSERT INTO poszt_hozzavalok (hozzavalo_id, poszt_id) VALUES (?, ?)",
                    [hozzavaloId, ujPosztId]
                );
            }
        }
